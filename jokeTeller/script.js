...
const audioElement=  document.querySelector('.audioConvertor');
...
const gettingSpeech= function(jokeString){

    VoiceRSS.speech({
        key: '6698e11b15f44a77a24be6940c462965',
        src:jokeString,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
const joke = async function(){
    try{
        const jokeString = await fetch('https://v2.jokeapi.dev/joke/Any?type=single');
        const data =  await jokeString.json();
        return data.joke
    }catch(error){
        console.log("The data cant be loaded ",error.message);
    }
...
buttonJoke.addEventListener('click',function(){
    joke().then(response=>{
        gettingSpeech(response);
        buttonJoke.disabled= true;
    })
})
audioElement.addEventListener('ended',function(){
    buttonJoke.disabled = false;
})
